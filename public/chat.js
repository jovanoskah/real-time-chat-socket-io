class Chat {
  ...
  showNewNotification(userId) {
    this.$usersList.querySelector(`div[data-id="${userId}"]`)
  ...
  activateChat($userElement) {
    ...
    if(this.activeChatId) {
      this.$usersList.querySelector(`div[data-id="${this.activeChatId}"]`)
    ...
    this.$textInput.addEventListener("keyup", (event) => {
      if(event.key === "Enter") {
        const message = {
          ...
          recepientId: this.activeChatId
        }
        socket.emit("new-chat-message", message);
        this.addMessage(message.text, message.recepientId)
        this.renderMessages(message.recepientId)
